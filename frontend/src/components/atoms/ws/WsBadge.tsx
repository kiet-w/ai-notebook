...
export default function WsBadge({ children, className = '' }: WsBadgeProps) {
  return (
    <span
      className={`inline-flex items-center gap-1 font-mono text-[10px] uppercase tracking-[0.15em] px-2 py-0.5 rounded-sm border border-blue-500/60 bg-blue-500/10 text-blue-400 select-none ${className}`}
    >
      {children}
    </span>
  );
