...
export function NoteDetailBody({ note, loading, hasDoc }: NoteDetailBodyProps) {
  ...
  const hasSidebar =
    hasDoc || !!note.summary || (!!note.bullets && note.bullets.length > 0);

  return (
    <div className="h-full">
      {hasSidebar ? (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-0 items-start lg:items-stretch h-full">
          {/* Left Column: Note Content */}
          <div className="lg:col-span-7 space-y-6 lg:h-full lg:overflow-y-auto no-scrollbar lg:p-8 lg:pr-4">
            <p className="text-[10px] font-bold text-zinc-400 dark:text-zinc-550 uppercase tracking-widest mb-2 select-none">
              {t('notes.fullContentAnalysis')}
            </p>
          ...
          {/* Right Column: Attachment, Summary & Bullets */}
          <div className="lg:col-span-5 space-y-6 lg:h-full lg:overflow-y-auto no-scrollbar lg:p-8 lg:pl-4 border-t lg:border-t-0 lg:border-l border-zinc-200/50 dark:border-zinc-800/40">
            ...
            {note.bullets && note.bullets.length > 0 && (
              <div>
                <p className="text-[10px] font-bold text-zinc-400 dark:text-zinc-550 uppercase tracking-widest mb-3 select-none">
                  {t('notes.keyTakeaways')}
                </p>
                <ul className="space-y-2.5">
                  {note.bullets.map((bullet, i) => (
                    <li
                      key={i}
                      className="text-base text-zinc-700 dark:text-zinc-300 flex items-start gap-3"
                    >
